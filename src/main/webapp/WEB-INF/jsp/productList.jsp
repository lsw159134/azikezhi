...
<div class="row">
	...
	<div class="col-sm-12" style="margin-left: 10px;">
		...
		<div class="col-sm-1">
			<a href="<%=basePath %>1019/zai/xin/jian/seleQuality" class="btn btn-primary">
				查询
			</a>
		</div>
		<form action="<%=basePath %>1019/zai/xin/jian/queryAllProduct" method="get">
				...
				<c:forEach items="${product}" var="product" varStatus="status">
							...
							<td width="60px;">
								...
								<c:if test="${product.p_data != null && product.product_miaoshu != null && product.p_data != '' && product.product_miaoshu != ''}">
									<a href="updateProduct?uid=${product.productuuid }" class="exchange"
								...
								<c:if test="${sessionScope.user.admin_jurisdiction == 7 }">
									<c:if test="${fn:contains(sessionScope.user.admin_shrio, 'o')}">
										|<a href="deleteProduct?productuuid=${product.productuuid }" class="exchange"
...
function first(){
	...
	window.location.href='<%=basePath %>1019/zai/xin/jian/queryAllProduct?page=${1}&&rows=${pageSize }&supplier=' + supplier+'&product=' + search+'&manufacturer='+manufacturer;
}
function end(){
	var manufacturer = $("#manufacturer").val();
	var supplier = $("#supplier").val();
	var search = $("#search").val();
	window.location.href='<%=basePath %>1019/zai/xin/jian/queryAllProduct?page=${count == 0 ? 1:count }&&rows=${pageSize }&supplier=' + supplier+'&product=' + search+'&manufacturer='+manufacturer;
}
function previous(){
	var page=${page == 1};
	var manufacturer = $("#manufacturer").val();
	...
	if(page){
		...
		window.location.href='<%=basePath %>1019/zai/xin/jian/queryAllProduct?page=${page-1}&&rows=${pageSize }&supplier=' + supplier+'&product=' + search+'&manufacturer='+manufacturer;
	}
}
function next(){
	var page=${count <= page};
...
function searchProduct() {
	...
	var search = $("#search").val();
	var supplier = $("#supplier").val();
	var url = '<%=basePath %>1019/zai/xin/jian/queryAllProduct?product=' + search+'&supplier='+supplier+'&manufacturer='+manufacturer;
	window.location = url;
};
