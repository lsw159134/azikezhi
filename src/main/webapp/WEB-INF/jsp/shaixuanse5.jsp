...
<html lang="lang=zh-cmn-Hans">
	...
	<body >
		...
		<div class="shaixuan" style="background: #F5F5F5;padding:20px 0px 20px 150px;">
			<c:if test="${product_second != null && manufacturer != null && primary_name != null}">
				<p style="margin-left: -130px">
					 您当前的筛选条件 
					 <span style="font-weight: bold;"><a href="shaixuanse?primary_name=${primary_name }"> - ${primary_name } </a></span>
					 <span style="font-weight: bold;"><a href="shaixuanse1?primary_name=${primary_name }&&product_second=${product_second }"> - ${product_second } </a></span>
					 <span style="font-weight: bold;"><a href="shaixuanse5?primary_name=${primary_name }&&product_second=${product_second }&&manufacturer=${manufacturer }"> - (品牌)${manufacturer } </a></span>
				</p>
			...
			<a href="shaixuanse3?primary_name=${primary_name }&&manufacturer=${manufacturer}">
				...
				<c:forEach items="${encapsulationList }" var="enList" varStatus="status">
					<c:if test="${status.count <= 24 }">
						<p  title="${enList.encapsulation }" style="display: inline-block;width: 170px;line-height: 30px;">
							<a class="endiva" 
							   href="shaixuanse7?primary_name=${primary_name }&&product_second=${product_second }&&encapsulation=${enList.encapsulation}&&manufacturer=${manufacturer }" 
			...
			<c:forEach items="${manufacturerList }" var="maList" varStatus="status">
				...
				<a href="shaixuanse1?primary_name=${primary_name }&&product_second=${product_second }&&ennum=${ennum }&&manum=${manum }">
					<img style="position: absolute;margin-top: 2px;" src="logo/clearshaixuan.png">
				</a>
			</c:forEach>
		</div>
