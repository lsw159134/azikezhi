		...
		<div class="shaixuan" style="background: #F5F5F5;padding:20px 0px 20px 150px;">
			<c:if test="${product_second != null || manufacturer != null || encapsulation != null || primary_name != null}">
				<p style="margin-left: -130px">
					 您当前的筛选条件 
					 <c:if test="${manufacturer != null }"><span style="font-weight: bold;"><a href="shaixuanse?brand_name=${manufacturer }"> - (品牌)${manufacturer } </a></span></c:if>
					 <c:if test="${encapsulation != null }"><span style="font-weight: bold;"><a href="shaixuanse6?manufacturer=${manufacturer }&&encapsulation=${encapsulation }"> - (封装)${encapsulation } </a></span></c:if>
				</p>
			...
			<c:forEach items="${encapsulationList }" var="enList" varStatus="status">
				...
				<a href="shaixuanse?brand_name=${manufacturer }">
	...
	function doSwithTab(pid){
		...
		$(function () {
			$("#page").paging({
	            ...
	            idParameter: 'page',
	            submitStyle: 'href',
				url: '<%=basePath%>shaixuanse6?manufacturer=${manufacturer}&encapsulation=${encapsulation}'
			});
		});
