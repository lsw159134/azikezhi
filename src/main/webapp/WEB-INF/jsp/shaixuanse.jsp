		...
		<div class="shaixuan" style="background: #F5F5F5;padding:20px 0px 20px 150px;">
			<c:if test="${product_second != null || manufacturer != null || encapsulation != null || primary_name != null}">
				<p style="margin-left: -130px">
					 您当前的筛选条件 
					 <c:if test="${manufacturer != null }"><span style="font-weight: bold;"><a href="shaixuanse?brand_name=${manufacturer }"> - ${manufacturer } </a></span></c:if>
				</p>
				...
				<c:forEach items="${encapsulationList }" var="enList" varStatus="status">
					<c:if test="${status.count <= 24 }">
						<p  title="${enList.encapsulation }" style="display: inline-block;width: 170px;line-height: 30px;">
							<a class="endiva" 
							   href="shaixuanse6?encapsulation=${enList.encapsulation}&&manufacturer=${manufacturer}" 
				...
				<c:forEach items="${manufacturerList }" var="maList" varStatus="status">
					<c:if test="${status.count <= 24 }">
						<p title="${maList.manufacturer }" style="display: inline-block;width: 200px;line-height: 30px;">
							<a class="madiva" href="shaixuanse3?primary_name=${primary_name }&&manufacturer=${maList.manufacturer}&&manum=${maList.product_primary }&&ennum=${ennum}" style="color: blue;">${maList.manufacturer } (${maList.product_primary })</a>
						</p>
					</c:if>
	...
	function doSwithTab(pid){
		...
		$(function () {
			$("#page").paging({
	            ...
	            idParameter: 'page',
	            submitStyle: 'href',
				url: '<%=basePath%>shaixuanse?brand_name=${manufacturer}'
			});
		});
