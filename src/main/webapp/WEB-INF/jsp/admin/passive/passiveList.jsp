...
<title>Insert title here</title>
	...
	<script src="<%=contextPath %>/js/paging.js" type="text/javascript"></script>
<style type="text/css">
...
<a name="top"></a>
	<div>
		<div class="cl pd-5 bg-1 bk-gray" style="height:40px;">
			<div class="col-sm-1" style="float: left;">
				<a href="addpassive" class="btn btn-primary">
					添加被动器件商家
				</a>
			</div>
			<form action="<%=basePath %>1019/zai/xin/jian/passiveList" method="get">
				<div class="col-sm-2" style="float: right;">
					<div class="form-group">
						<div class="input-group">
							...
							<input type="search" style="border:2px double rgb(250,0,255);height: 30px;" class="list-group-item list-group-item-success" placeholder="分类名称"
	...
	<div class="mt-20" style="background:#CBE9CF; color:#00060B">
		<table class="table table-striped table-hover" id="sample-table-2">
			...
			<tbody>
				<c:forEach items="${passive}" var="passive" varStatus="status">
					<tr height="60px;">
						<td>${passive.down}</td>
						<td><p style="margin-top:0px;">${passive.name}</p></td>
						<td><p style="margin-top:0px;">${passive.phone1}</p><p style="margin-top: 0px;">${passive.phone2}</p></td>
						<td>
							<p style="width: 200px;overflow: hidden;clear:both;text-overflow:ellipsis;white-space: nowrap;text-align: left;" title="${passive.address}">${passive.address}</p>
							<p style="margin-top:0px;width: 200px;overflow: hidden;clear:both;text-overflow:ellipsis;white-space: nowrap;text-align: left" title="${passive.illustration}">${passive.illustration}</p>
						</td>
						<td class="fan"><p style="margin-top:0px;">${passive.cellphone}</p></td>
						<td align="center">
							<p style="margin-top:0px;">${passive.qq1}</p>
							<p style="margin-top: 0px;">${passive.qq2}</p>
						</td>
						<td align="center">
							<p style="margin-top: 0px;">${passive.subcategory.subcategory_name}</p>
						</td>
						<td>
							<p style="margin-top:0px;"><fmt:formatDate value="${passive.time}" pattern="yyyy-MM-dd HH:mm"/></p>
						</td>
						<td align="center">
							<a href="uppassive?pid=${passive.passivecomponentsuid}">修改</a>|
							<a href="deletePassiveComponents?pid=${passive.passivecomponentsuid}" onClick="return confirm('确定删除?');" >删除</a>
						</td>
					</tr>
				</c:forEach>
			</tbody>
	...
	<div style="width: 100%;height: 27px;margin-top: 3px;">
       	...
       	<input size="1px;" class="btn btn-secondary size-MINI radius" onclick = "san()"<c:if test='${count <= (page+1)}'> style="opacity: 0.2;" </c:if> value="${page+2 }" >&nbsp;
    ...
    <div id="updialog">  
    	...
    	<form id="quoteForm" action="supplierupdateByAdmin" method="post" style="padding-left: 20px;padding-top: 20px;">
	    	...
	    	<p>
	    		标&#8195;签&#8195;1&#8195;
	    	</p><input onblur="id1()" type="text" maxlength="4" id="supplier_label1" name="supplier_label1" placeholder="标签" style="text-transform: uppercase;width: 240px;color:#000000;">
	    	<p style="display: block;margin-left: 80px;height: 5px;color: red;"></p>
	    	<p>
	    		标&#8195;签&#8195;2&#8195;
	    	</p><input onblur="id1()" type="text" maxlength="4" id="supplier_label2" name="supplier_label2" placeholder="标签" style="text-transform: uppercase;width: 240px;color:#000000;">
	    	<p style="display: block;margin-left: 80px;height: 5px;color: red;"></p>
	    	<p>
	    		标&#8195;签&#8195;3&#8195;
	    	</p><input onblur="id1()" type="text" maxlength="4" id="supplier_label3" name="supplier_label3" placeholder="标签" style="text-transform: uppercase;width: 240px;color:#000000;">
	    	<p style="display: block;margin-left: 80px;height: 5px;color: red;"></p>
	    	<p>
	    		标&#8195;签&#8195;4&#8195;
	    	</p><input onblur="id1()" type="text" maxlength="4" id="supplier_label4" name="supplier_label4" placeholder="标签" style="text-transform: uppercase;width: 240px;color:#000000;">
	    	<p style="display: block;margin-left: 80px;height: 5px;color: red;"></p>
	    	<p>
	    		标&#8195;签&#8195;5&#8195;
...
function searchProduct() {
	var name=$("#name").val();
	var supplier = $("#product").val();
	var url = '<%=basePath %>1019/zai/xin/jian/passiveList?product='+supplier+'&name='+name;
	window.location = url;
};
...
function first(){
	window.location.href='passiveList?page=${1}&product=${product}';
}
function end(){
	window.location.href='passiveList?page=${count == 0 ? 1:count }&product=${product}';
}
function er(){
	var page=${page+1 };
	var count=${count };
	if(page > count){
		...
		window.location.href='passiveList?page=${page+1}&product=${product}';
	}
}
function san(){
	var page=${page+2 };
	...
	if(page > count){
		...
		window.location.href='passiveList?page=${page+2}&product=${product}';
	}
}
function si(){
	var page=${page+3 };
	...
	if(page > count){
		...
		window.location.href='passiveList?page=${page+3}&product=${product}';
	}
}
function previous(){
	var page=${page == 1};
