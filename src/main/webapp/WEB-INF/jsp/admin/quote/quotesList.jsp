...
<a name="top"></a>
	<div>
		<div class="cl pd-5 bg-1 bk-gray" style="height:40px;">
			<form action="quoteAllAdmin" method="post">
				<input type="text" name="product" placeholder="请输入型号" style="height: 30px;border: 2px #87CEEB solid;-moz-border-radius:2px;-webkit-border-radius:2px;border-radius:2px;">
				<select name="status" style="height: 30px;border: 2px #87CEEB solid;-moz-border-radius:2px;-webkit-border-radius:2px;border-radius:2px;text-align: center;">
			...
			<div class="mt-20">
				<table style="border: 1px #EEEEEE solid;border-collapse: collapse;width: 1127px;margin-left: 0px;">
					<thead  style="background: #BCBFFD">
						<tr >
							<th width="80" style="height: 40px;color: black;font-weight: bold;text-align: center;">会员</th>
							<th width="160" style="height: 40px;color: black;font-weight: bold;text-align: center;">产品型号</th>
							<th width="160" style="height: 40px;color: black;font-weight: bold;text-align: center;">供应商</th>
							<th style="height: 40px;color: black;font-weight: bold;text-align: center;">联系方式</th>
							<th style="height: 40px;color: black;font-weight: bold;text-align: center;">品牌</th>
							<th style="height: 40px;color: black;font-weight: bold;text-align: center;">批号</th>
							...
							<th style="height: 40px;color: black;font-weight: bold;text-align: center;">价格</th>
							<th style="height: 40px;color: black;font-weight: bold;text-align: center;">质量</th>
					...
					<tbody >
						<c:forEach items="${quote}" var="quote" varStatus="status">
							<tr style="<c:if test='${status.count%2==0 }'>background:#F3F3F3</c:if>">
								<td style="height: 40px;width: 80px;border-right: 2px solid #EEEEEE;" title="">
									<p style="width: 80px;overflow: hidden;clear:both;text-overflow:ellipsis;white-space: nowrap;text-align: left;padding-left: 5px;" title="${quote.user.username}">${quote.user.username}</p>
								</td>
								<td style="height: 40px;width: 140px;border-right: 2px solid #EEEEEE;"><p style="padding-left: 10px;width: 140px;overflow: hidden;clear:both;text-overflow:ellipsis;white-space: nowrap;text-align: left;" title="${quote.quote_product}"><a href="search?keyword=${quote.quote_product}" target="_blank">${quote.quote_product}</a></p></td>
								<td style="height: 40px;width: 140px;border-right: 2px solid #EEEEEE;" title="">
									<p style="width: 140px;overflow: hidden;clear:both;text-overflow:ellipsis;white-space: nowrap;text-align: left;padding-left: 5px;" title="${quote.quote_name}">${quote.quote_name}</p>
								</td>
								<td style="height: 40px;text-align: center;border-right: 2px solid #EEEEEE;" title="">
									${quote.quote_telephone}
								</td>
								<td style="height: 40px;text-align: center;border-right: 2px solid #EEEEEE;" title="${quote.quote_brand}">${quote.quote_brand}</td>
								<td style="height: 40px;text-align: center;border-right: 2px solid #EEEEEE;" title="${quote.quote_lotnumber}">${quote.quote_lotnumber}</td>
								<td style="height: 40px;text-align: center;border-right: 2px solid #EEEEEE;" title="${quote.quote_package}">${quote.quote_package}</td>
								<td style="height: 40px;text-align: center;border-right: 2px solid #EEEEEE;" title="${quote.quote_number}">${quote.quote_number}${quote.quote_utilnumber}</td>
								<td style="height: 40px;text-align: center;border-right: 2px solid #EEEEEE;width: 80px;" title="${quote.quote_price}"><c:if test="${quote.quote_priceutil eq '美元' }">$</c:if><c:if test="${quote.quote_priceutil ne '美元' }">￥</c:if>${quote.quote_price}</td>
								<td style="height: 40px;text-align: center;border-right: 2px solid #EEEEEE;" title="${quote.quote_quality}">${quote.quote_quality}</td>
								<td style="height: 40px;text-align: center;border-right: 2px solid #EEEEEE;width: 140px;">
									<fmt:formatDate value="${quote.quote_date}" pattern="yyyy-MM-dd HH:mm"/>
								</td>
								<td style="height: 40px;text-align: center;width: 60px;border-left: 2px solid #EEEEEE;">
									<c:if test="${quote.quote_status==1}">
										报价中
									</c:if>
									<c:if test="${quote.quote_status==2}">
				...
				<div class="g-cf g-pagerwp g-bd-box g-plr-10 g-o-h" style="width: 1113px;margin-left: 20px;">
					<div class="g-pager g-f-r" style="visibility: visible;">
						...
						<c:if test="${page>1}">
							<a href="queryByquote">首页</a><a href="queryByquote?page=${page-1 }">上一页</a>
						</c:if>
						<c:if test="${page==count || count==0}">
							<a class="no">下一页</a><a class="no">尾页</a>
						</c:if>
						<c:if test="${page!=count && count!=0}">
							<a href="queryByquote?page=${page+1 }">下一页</a><a href="queryByquote?page=${count }">尾页</a>
						</c:if>
						<c:if test="${page>3}">
