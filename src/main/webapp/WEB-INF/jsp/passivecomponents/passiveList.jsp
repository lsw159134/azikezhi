			...
			<tbody>
				<c:forEach items="${passive}" var="passive" varStatus="status">
					<tr height="60px;">
						<td>${passive.down}</td>
						<td>${passive.name}</td>
						<td>${passive.phone1}<p style="margin-top: 0px;">${passive.phone2}</p></td>
						<td>
						...
						<td class="fan">${passive.cellphone}</td>
						<td align="center">
							${passive.qq1}
							<p style="margin-top: 0px;">${passive.qq2}</p>
						</td>
						<td>
							<fmt:formatDate value="${passive.time}" pattern="yyyy-MM-dd HH:mm"/>
						</td>
						<td align="center">
...
function searchProduct() {
	...
	var url = '<%=basePath %>1019/zai/xin/jian/supplierList?suppliername='+supplier;
	window.location = url;
};
