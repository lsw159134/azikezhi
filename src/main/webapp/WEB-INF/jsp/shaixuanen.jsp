...
<html lang="lang=zh-cmn-Hans">
	...
	<body >
		...
		<div class="shaixuan" style="background: #F5F5F5;padding:20px 0px 20px 150px;">
			<c:if test="${ encapsulation != null }">
				<p style="margin-left: -130px">
					 您当前的筛选条件 
					<span style="font-weight: bold;"><a href="shaixuanen?keyword=${keyword }&&encapsulation=${encapsulation }"> - (封装)${encapsulation } </a></span>
				</p>
			...
			<c:if test="${maSize > 0 }">
				...
				<c:forEach items="${manufacturerList }" var="maList" varStatus="status">
					<c:if test="${status.count <= 24 }">
						<p title="${maList.brand.brand_name }" style="display: inline-block;width: 200px;line-height: 30px;">
							<a href="shaixuanenma?keyword=${keyword }&manufacturer=${maList.brand.brand_name}&&encapsulation=${encapsulation}" style="color: blue;">${maList.brand.brand_name } (${maList.product_primary })</a>
						</p>
					</c:if>
...
<script type="text/javascript">
	...
	$(function () {
		$("#page").paging({
            ...
            idParameter: 'page',
            submitStyle: 'href',
			url: '<%=basePath%>shaixuanen?keyword=${keyword}&product_second=${product_second}&value=product&manufacturer=${manufacturer}&encapsulation=${encapsulation}'
		});
	});
