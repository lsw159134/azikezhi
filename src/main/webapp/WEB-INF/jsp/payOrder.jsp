	...
	<div class="payment pay-load">
	    ...
	    <div id="payChanel" class="pay-channel animate-enter animate-enter-active">
	    	<div style="margin-left: 350px;">
		        <p style="font-size: 16px;margin-top: -5px;margin-left: -160px;">本站所有商品均为自取。取货地址：新华强二楼Q2B051；联系电话：0755-82720040</p>
		    ...
		    <div style="margin-top: 20px;margin-left: 330px;">
		    	...
		    	<a class="fu" onclick="saveCart('${eid }')" href="javascript:void(0)" style="color: white; position: relative; background: #ADADAD; padding: 9px 26px; font-weight: bold; border-radius: 5px;width: 100px;height: 28px;">稍后支付</a>
				<a class="fu1" onclick="Cart('${eid }')" href="javascript:void(0)" style="margin-left: 10px; color: white; position: relative; background: #FF2D2D; padding: 9px 26px; font-weight: bold; border-radius: 5px;margin-left:56px;">付款成功</a>
			</div>
		</div>
		<br><br><br>
	</div>
...
$('.fukuan').hover(function(){
	...
	function Cart(express){
		var mode=$("#mode").val();
		 $.post("indentdelect?express="+express+"&biaozi=2"+"&mode="+mode,
            function(data){
				if(data.result=="fail"){
