	...
	<div class="col-sm-12" style="margin-left: 10px;margin-top: -20px">
		<c:if test="${sessionScope.user.admin_status eq '1' || user.admin_status eq '' || user.admin_status eq null }">
			<p style="padding-bottom: 10px;padding-left: 20px">
				<span style="font-weight: bolder;font-size: 20px"> 总共   </span> 的任务还剩下 : <span style="font-weight: bolder;font-size: 20px"> ${total} </span> 条
			</p>
			<div class="col-sm-1">
			...
			<div class="col-sm-1">
				<a href="nullData?kongzhi=123" class="btn btn-primary">
					添加图片
				</a>
			</div>
		</c:if>
							...
							<tr height="60px;" class="text-c">
								...
								<c:if test="${sessionScope.user.admin_status eq '2' || user.admin_status eq '' || user.admin_status eq null }">
									<td width="60px;">
										...
										<br>
										<a href="seleNullData?uid=${product.productuuid}" class="exchange"
										style="padding-right: 0px;width: 20px;color: blue;">修改</a>
										
										|<a href="deleteNullProduct?productuuid=${product.productuuid }" class="exchange"
...
<script>
function first(){
	window.location.href='nullData?page=${1}&&rows=${pageSize }';
}
function end(){
	var supplier = $("#supplier").val();
	var search = $("#search").val();
	window.location.href='nullData?page=${count == 0 ? 1:count }&&rows=${pageSize }';
}
function previous(){
	var page=${page == 1};
	var supplier = $("#supplier").val();
	...
	if(page){
		...
		window.location.href='nullData?page=${page-1}&&rows=${pageSize }';
	}
}
function next(){
	var page=${count <= page};
...
function searchProduct() {
	...
	var url = 'nullData?manufacturer=' + manufacturer;
	window.location = url;
};
