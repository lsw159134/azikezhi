    ...
    <div class="main">
		...
		<br>
		        <p class="o-tips" id="deleteOrderTip" style="font-size: 16px;">
		                           请您在30分钟内完成支付，否则订单会被自动取消(库存紧俏订单请参见详情页时限)
		                    </p>
		    </div>
		    <!-- 订单信息 end --><!-- 订单金额 -->
		    <div class="o-right">
	...
	<div class="payment pay-load">
	    ...
	    <div id="payChanel" class="pay-channel animate-enter animate-enter-active">
	    	<div style="margin-left: 350px;">
		        <p style="font-size: 16px;margin-top: -5px;margin-left: -160px;">本站所有商品均为自取。取货地址：以订单的收货地址为标准；联系电话：0755-82720040</p>
		    ...
		    <div id="zhifubao" style="margin-left: 330px;margin-top: 20px;">
		    	...
		    	<span style="font-size: 14px;margin-left: 120px;">亲，付款成功后</span>
		    </div>
		    <div style="float: right;margin-right: 35px;margin-top: -140px;font-size: 14px;">请记得点击下方<b style="color:red;">付款成功</b>按钮</div>
		    <div style="margin-top: 20px;margin-left: 330px;">
		    	...
		    	<a class="fu" onclick="saveCart('${eid }','${panduan }')" href="javascript:void(0)" style="color: white; position: relative; background: #ADADAD; padding: 9px 26px; font-weight: bold; border-radius: 5px;width: 100px;height: 28px;font-size: 14px;">稍后支付</a>
				<a class="fu1" onclick="Cart('${eid }','${panduan }')" href="javascript:void(0)" style="margin-left: 10px; color: white; position: relative; background: #FF2D2D; padding: 9px 26px; font-weight: bold; border-radius: 5px;margin-left:38px;font-size: 14px;">付款成功</a>
			</div>
		</div>
		<br><br><br>
	</div>
...
$('.fukuan').hover(function(){
	...
	function Cart(express,panduan){
		if(panduan!=""){ 
		   $.post("indentdelect?bdexpress="+express+"&biaozi=2",
              function(data){
				  if(data.result=="fail"){
		...
		$.post("indentdelect?zdexpress="+express+"&biaozi=2",
			   function(data){
				if(data.result=="fail"){
				}else if(data.result=="seccuss"){
				window.location.href='express';
