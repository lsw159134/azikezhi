				...
				<dd style="display: block;">
					...
					<li>
						...
						<li style="background: #D0DFEF;">
							<a href="beidongExpress" style="margin-left: -25px" data-title="被动元器件订单">
								<img style="position: absolute;margin-top: 8px;" src="<%=basePath%>logo/yuan.png?3214">&emsp;&emsp;被动元器件订单 
								...
								<tr>
									...
									<td style="width: 100px; padding: 20px 0px 20px 3px; border: 1px #AED8FF solid;"valign="top">
										<c:if test="${items.status == 1}">
											<c:if test="${items.preferentialamount==null}">
												<a class="fukuan" href="houfu?eid=${items.express}&price=${items.eprice}&ordernumber=${items.ordernumber}&panduan=beidong"
											...
											<c:if test="${items.preferentialamount!=null}">
												<a class="fukuan" href="houfu?eid=${items.express}&price=${items.preferentialamount}&ordernumber=${items.ordernumber}&panduan=beidong"
													style="color: white; position: relative; background: #EE4000; padding: 6px 14px 6px 14px; font-weight: bold; -moz-border-radius: 5px; -webkit-border-radius: 5px; border-radius: 5px;">
													立即付款
												</a>
											</c:if>
