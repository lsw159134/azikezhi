...
<html lang="lang=zh-cmn-Hans">
	...
	<body >
		<div class="shaixuan" style="background: #F5F5F5;padding:20px 0px 20px 150px;">
			<c:if test="${product_second != null || manufacturer != null || encapsulation != null || primary_name != null}">
				<p style="margin-left: -130px">
					 您当前的筛选条件 
					 <c:if test="${primary_name != null }"><span style="font-weight: bold;"><a href="shaixuanse?primary_name=${primary_name }"> - ${primary_name } </a></span></c:if>
					 <c:if test="${encapsulation != null }"><span style="font-weight: bold;"><a href="shaixuanse2?primary_name=${primary_name }&&encapsulation=${encapsulation}"> - (封装)${encapsulation }</a></span></c:if>
				</p>
			...
			<c:if test="${seSize > 0 }">
				...
				<c:forEach items="${secondnameList }" var="seList" varStatus="status">
					<c:if test="${status.count <= 24 }">
						<p  title="${seList.second_name }" style="display: inline-block;width: 170px;line-height: 30px;">
							<a href="shaixuanse1?primary_name=${primary_name }&&product_second=${seList.second_name }" 
							   style="color: blue;">${seList.second_name }</a>
						</p>
					</c:if>
					<c:if test="${status.count > 24 }">
						<p class="moreSe" title="${seList.second_name }" style="display: none;width: 170px;line-height: 30px;">
							<a class="endiva" 
							   href="shaixuanse1?primary_name=${primary_name }&&product_second=${seList.second_name }" 
							   style="color: blue;">${seList.second_name }</a>
						</p>
					</c:if>
				</c:forEach>
				<c:if test="${seSize > 24 }">
					<p>
						<a href="javascript:void(0)" class="moreSeShow">更多<img style="position: absolute;" src="logo/moredown.png"></a>
					</p>
				</c:if>
				<div style="width: 1200px;height: 0;border-bottom: #A1A1A1 1px dashed;margin-left: -150px;margin-bottom: 10px;margin-top: 10px;"></div>
			</c:if>
			...
			<c:forEach items="${encapsulationList }" var="enList" varStatus="status">
				<p title="${encapsulation }" style="min-width: 160px;line-height: 30px;border: 1px #FFA500 solid;text-align: center;display: inline-block;">
					${encapsulation } (${enList.product_primary })&nbsp;&nbsp;
				</p>
				<a href="shaixuanse?primary_name=${primary_name }">
			...
			<c:if test="${maSize > 0 }">
				...
				<c:forEach items="${manufacturerList }" var="maList" varStatus="status">
					<c:if test="${status.count <= 24 }">
						<p title="${maList.manufacturer }" style="display: inline-block;width: 200px;line-height: 30px;">
							<a class="madiva" href="shaixuanse6?primary_name=${primary_name }&&manufacturer=${maList.manufacturer}&&encapsulation=${encapsulation}" style="color: blue;">${maList.manufacturer } (${maList.product_primary })</a>
						</p>
					</c:if>
					
					<c:if test="${status.count > 24 }">
						<p class="moreMa" title="${enList.encapsulation }" style="display: none;width: 200px;line-height: 30px;">
							<a class="madiva" href="shaixuanse6?primary_name=${primary_name }&&manufacturer=${maList.manufacturer}&&encapsulation=${encapsulation}" style="color: blue;">${maList.manufacturer } (${maList.product_primary })</a>
						</p>
					</c:if>
				</c:forEach>
				<c:if test="${maSize > 24 }">
					<p >
						<a href="javascript:void(0)" class="moreMaShow">更多<img style="position: absolute;" src="logo/moredown.png"></a>
					</p>
				</c:if>
			</c:if>
		</div>
